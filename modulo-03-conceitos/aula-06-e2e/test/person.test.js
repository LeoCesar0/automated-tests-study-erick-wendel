...
describe("#Person Suite", () => {
  ...
  describe("#validate", () => {
    it("should throw if name is not defined", () => {
      ...
      expect(() => Person.validate(person)).toThrow(
        new Error(PERSON_EXCEPTIONS.NAME_NOT_PROVIDED)
      );
    });
    it("should throw if cpf is not defined", () => {
      ...
      // Assert
      expect(() => Person.validate(person)).toThrow(
        new Error(PERSON_EXCEPTIONS.CPF_NOT_PROVIDED)
      );
    });
  ...
  describe("#process", () => {
    it("should correctly process a person", () => {
      ...
      const processResult = Person.process({
        name: "Fulano de Tal",
        cpf: "123.123.123-12",
      });
      ...
      expect(processResult).toStrictEqual(expected);
    });
  });
});
